...
ENV SCONE_HEAP=4G SCONE_FORK=0 SCONE_ALLOW_DLOPEN=2 SCONE_MPROTECT=1 SCONE_SYSLIBS=1 SCONE_LOG=ERROR
...
# resolve this by extracting the library beforehand into /usr/lib. same thing with liblz4, common-crypto and libsnappy.
RUN apk add --no-cache --update build-base jpeg-dev zlib-dev openssl-dev && \
    pip install matplotlib && \
    scone-signer sign --env /usr/lib/jvm/java-1.8-openjdk/bin/java && \
    ...
    rm -rf /tmp/* && \
    unzip /spark/jars/commons-crypto-1.1.0.jar && cp org/apache/commons/crypto/native/Linux/x86_64/libcommons-crypto.so /usr/lib && \
    rm -rf /tmp/* && \
    unzip /spark/jars/snappy-java-1.1.8.2.jar && cp org/xerial/snappy/native/Linux/x86_64/libsnappyjava.so /usr/lib && \
    rm -rf /tmp/* 
